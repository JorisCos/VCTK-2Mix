...
parser.add_argument('--metadata_dir', type=str, required=True,
                    help='Path to the VCTKMix metadata directory')
parser.add_argument('--VCTKmix_outdir', type=str, default=None,
...
def main(args):
    ...
    # Get VCTKMix root path
    VCTKmix_outdir = args.VCTKmix_outdir
    n_src = args.n_src
    if VCTKmix_outdir is None:
        VCTKmix_outdir = os.path.dirname(metadata_dir)
    VCTKmix_outdir = os.path.join(VCTKmix_outdir, f'VCTK{n_src}Mix')
    ...
    create_VCTKmix(VCTK_dir, wham_dir, VCTKmix_outdir, metadata_dir,
                   freqs, n_src, modes, types)


def create_VCTKmix(VCTK_dir, wham_dir, out_dir, metadata_dir,
                   freqs, n_src, modes, types):
    """ Generate sources mixtures and saves them in out_dir"""
    # Get metadata files
    md_filename_list = [file for file in os.listdir(metadata_dir)
                        if 'info' not in file]
    # Create all parts of VCTKmix
    for md_filename in md_filename_list:
        csv_path = os.path.join(metadata_dir, md_filename)
        process_metadata_file(csv_path, freqs, n_src, VCTK_dir,
                              wham_dir, out_dir, modes, types)


def process_metadata_file(csv_path, freqs, n_src, VCTK_dir, wham_dir,
                          out_dir, modes, types):
    ...
    for freq in freqs:
        ...
        for mode in modes:
            ...
            dir_name = os.path.basename(csv_path).replace(
                f'VCTK{n_src}mix_', '').replace('-clean', '').replace(
                '.csv', '')
            ...
            process_utterance(md_file, VCTK_dir, wham_dir, freq, mode,
                              subdirs, dir_path, subset_metadata_path, n_src)


def process_utterance(md_file, VCTK_dir, wham_dir, freq, mode, subdirs,
    ...
    for index, row in tqdm(md_file.iterrows(), total=len(md_file)):
        ...
        mix_id, gain_list, sources = read_sources(row, n_src, VCTK_dir,
                                                  wham_dir)
        # Transform sources
...
def read_sources(row, n_src, VCTK_dir, wham_dir):
...
def transform_sources(sources_list, freq, mode, gain_list):
    """ Transform VCTK sources to VCTKmix """
    # Normalize sources
